...
nest_species <- eggs_sum %>%
  left_join(nest_data, join_by(Site, Year, Nest_ID))
```
...
snow_cover <-  read_csv(here(path_processed, snowcover_csv))
...
```{r}
snowcover_data <- read_csv(file.path(path_processed, snowcover_csv))
...
```{r}
snowcover_data_fixed <- snow_cover %>%
...
snowcover_month <- snowcover_data_fixed %>%
  ...
  group_by(Site, Month) %>% 
  summarise(avg_snow_cover = mean(Snow_cover, na.rm = TRUE))
